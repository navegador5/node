...
function setupMethodsOnProcess() {
...
function exposeAndDefine() {
    if (!config.noBrowserGlobals) {
      ...
      const {MessageChannel,MessagePort,MessageEvent} = require('internal/worker/io');
...
function setProcessAllowedNodeEnvironmentFlags() {
...
function setupTaskQueueAndTimers() {
  // We initialize the tick callbacks and the timer callbacks last during
  // bootstrap to make sure that any operation done before this are synchronous.
  // If any ticks or timers are scheduled before this they are unlikely to work.
  const dflt = setupTaskQueue();
  //default queue
  process.nextTick = dflt.nextTick;
  process._tickCallback = dflt.runNextTicks;
  process.dflt = dflt;
  //customer queue
  process.js = setupTaskQueue('js');
  process.html  = setupTaskQueue('html');
  process.css   = setupTaskQueue('css');
  process.cuqueues = cuqueues;
  //
  process.kTickClock = kTickClock;
  //
  const { getTimerCallbacks } = require('internal/timers');
  const { setupTimers } = internalBinding('timers');
  const { processImmediate, processTimers } = getTimerCallbacks(dflt.runNextTicks);
  ...
  /*
  function immediate_observer(){
      process.__immediate$trap__ = getLibuvNow()
      console.log("process immediate happened at: " + process.__immediate$trap__)
  }
  function timer_observer(){
      process.__timer$trap__ = getLibuvNow()
      console.log("process timers happened at: " + process.__timer$trap__)
  }
  setupTimers(immediate_observer,timer_observer)
  */
  // Note: only after this point are the timers effective

  ////
  process.getTimerCallbacks = getTimerCallbacks
  process.setupTimers = setupTimers
  process.processImmediate = processImmediate
  process.processTimers = processTimers
  ////
}
...
js_transferable.setup();
setupMethodsOnProcess();
setCredentials();
async_wrap.setupHooks(nativeHooks);
exposeAndDefine();
setTraceCategoryStateUpdateHandler(perThreadSetup.toggleTraceCategoryState);
setProcessAllowedNodeEnvironmentFlags();
setProcessAssert();
setProcessFeatures();
setException();
process.emitWarning = emitWarning;
setupTaskQueueAndTimers();
