...
function _defaultEval(that,pause,unpause,code, context, file, cb,prioritizedSigintQueue) {
    ...
    run_in_context_and_handle_top_await(
        that,err,context,script,awaitPromise,
        pause,unpause,cb,prioritizedSigintQueue
    )
    ////
}
...
function _onLine(self,cmd,sigd,paused) {
    ////important
    if(paused){return}
...
function _wrap_readline_tty_factory(self,options,paused,pausedBuffer,sigd,ttyWrite) {
    let preview;
    if(options.terminal) {
        preview= options.terminal && (options.preview !== undefined ? !!options.preview : !options.eval);
    } else {
...
function _createContext(that) {
    ...
    ////
    if(that.output === process.stdout) {
    } else {
        server.repl.repl.remotes.push(that);
        context.repl.repl.console_server = server;
    }
    ////
...
function REPLServer(
    ...
    self.on('line', function onLine(cmd) {
        _onLine(self,cmd,sigd,paused);
    });
    ////sig cont
    self.on('SIGCONT', function onSigCont() {_onSigCont(self)});
    ////tty write bind 
    const ttyWrite = FunctionPrototypeBind(self._ttyWrite, self);
    self._ttyWrite = _wrap_readline_tty_factory(self,options,paused,pausedBuffer,sigd,ttyWrite);
    ////
    self.displayPrompt();
}
